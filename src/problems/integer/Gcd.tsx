...
const Gcd = () => {
  ...
  const problems = useMemo(
    () => generateGcdProblems(seed, nmin, nmax),
    [seed, nmin, nmax],
  );
...
const renderLadder = (p: GcdProblem, showAnswers: boolean) => (
  <table className={`integer-ladder${showAnswers ? "" : " integer-hidden"}`}>
    <tbody>
      ...
      <tr>
        ...
        <td className="integer-ladder-cell integer-ladder-bottom">{p.ladderBottom[0]}</td>
        <td className="integer-ladder-cell integer-ladder-bottom">{p.ladderBottom[1]}</td>
      </tr>
    </tbody>
  </table>
);

const renderAnswerText = (p: GcdProblem, showAnswers: boolean) => {
