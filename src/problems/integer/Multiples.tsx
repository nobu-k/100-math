...
import { generateMultiplesProblems, DEFAULTS, PARAM_KEYS } from "./generators";
import "../../App.css";

const defaults = DEFAULTS.multiples;
...
const Multiples = () => {
  ...
  const problems = useMemo(
    () => generateMultiplesProblems(seed, nmin, nmax, count),
    [seed, nmin, nmax, count],
  );

  const qrUrl = useMemo(
    () => buildQrUrl(seed, nmin, nmax, count),
    [seed, nmin, nmax, count],
  );

  const handleNewProblems = useCallback(() => {
    const newSeed = randomSeed();
    updateUrl(newSeed, false, nmin, nmax, count);
    setSeed(newSeed);
  ...
  const handleToggleAnswers = useCallback(() => {
    setShowAnswers((prev) => {
      updateUrl(seed, !prev, nmin, nmax, count);
      return !prev;
    });
  }, [seed, nmin, nmax, count]);

  const regenerate = useCallback((nextMin: number, nextMax: number, nextCount: number) => {
    const newSeed = randomSeed();
    setSeed(newSeed);
    setShowAnswers(false);
    updateUrl(newSeed, false, nextMin, nextMax, nextCount);
  }, []);
...
const parseInitialState = () => {
  ...
  if (parsedSeed === null) updateUrl(seed, showAnswers, nmin, nmax, count);
