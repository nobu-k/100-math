...
import { type IntegralPolyMode, generateIntegralPoly } from "./integral-poly";
import { renderExprProblems } from "../equations/renderExprProblems";

const DEF = { mode: "mixed" as IntegralPolyMode };
const PARAM_KEYS = ["mode"];

const IntegralPoly = () => {
  const [initial] = useState(() => {
    const p = new URLSearchParams(window.location.search);
    return { mode: parseEnum(p.get("mode"), ["indefinite", "definite", "area", "mixed"] as const, DEF.mode) };
  });
  ...
  const onModeChange = useCallback((v: IntegralPolyMode) => {
    setMode(v);
  ...
  const problems = useMemo(() => generateIntegralPoly(seed, mode), [seed, mode]);

  const settingsPanel = (
    <div className="no-print settings-panel">
      <label>
        種類{" "}
        <select className="operator-select" value={mode} onChange={(e) => onModeChange(e.target.value as IntegralPolyMode)}>
