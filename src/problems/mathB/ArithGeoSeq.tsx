...
import { type SeqMode, generateArithGeoSeq } from "./arith-geo-seq";
import { renderExprProblems } from "../equations/renderExprProblems";

const DEF = { mode: "mixed" as SeqMode };
const PARAM_KEYS = ["mode"];

const ArithGeoSeq = () => {
  const [initial] = useState(() => {
    const p = new URLSearchParams(window.location.search);
    return { mode: parseEnum(p.get("mode"), ["arithmetic", "geometric", "sigma", "mixed"] as const, DEF.mode) };
  });
  ...
  const onModeChange = useCallback((v: SeqMode) => {
    setMode(v);
  ...
  const problems = useMemo(() => generateArithGeoSeq(seed, mode), [seed, mode]);

  const settingsPanel = (
    <div className="no-print settings-panel">
      <label>
        種類{" "}
        <select className="operator-select" value={mode} onChange={(e) => onModeChange(e.target.value as SeqMode)}>
          <option value="mixed">すべて</option>
          <option value="arithmetic">等差数列</option>
          <option value="geometric">等比数列</option>
          <option value="sigma">Σ計算</option>
        </select>
      </label>
    </div>
  );
